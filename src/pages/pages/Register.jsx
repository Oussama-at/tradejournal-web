...
const TERMS_STYLE = {
  row:  { display: 'flex', alignItems: 'flex-start', gap: 10, margin: '14px 0', fontSize: 13, lineHeight: 1.5 },
  box:  { marginTop: 2, width: 16, height: 16, flexShrink: 0, cursor: 'pointer' },
  text: { cursor: 'pointer' },
  link: { color: '#00e676', textDecoration: 'underline', cursor: 'pointer' },
};
...
export default function Register() {
  ...
  async function finalSubmit() {
    if (!agree) {
      setMsg({ type: 'error', text: 'Please accept the Terms & Conditions and Privacy Policy to continue.' });
      return;
    }
    setLoading(true);
    setMsg(null);
      ...
      <div className="login-box" style={{ maxWidth: 520 }}>
        ...
        {((step === 3 && isFree) || (step === 4 && !isFree)) && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={TERMS_STYLE.row}>
              <input type="checkbox" id="agree-terms" checked={agree} onChange={e => setAgree(e.target.checked)} style={TERMS_STYLE.box} />
              <label htmlFor="agree-terms" style={TERMS_STYLE.text}>
                I have read and accept the{' '}
                <span style={TERMS_STYLE.link} onClick={(e) => { e.stopPropagation(); e.preventDefault(); window.open('/terms', '_blank'); }}>Terms & Conditions</span>
                {' '}and{' '}
                <span style={TERMS_STYLE.link} onClick={(e) => { e.stopPropagation(); e.preventDefault(); window.open('/privacy', '_blank'); }}>Privacy Policy</span>.
              </label>
            </div>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button className="btn btn-primary" style={{ flex: 2, justifyContent: 'center', padding: 13 }}
                onClick={finalSubmit} disabled={loading || !agree}>
